...
describe('Performance Benchmarks', () => {
  ...
  describe('Sync Engine Performance', () => {
    it('should handle 1000 sync operations efficiently', async () => {
      ...
      for (let i = 0; i < operations; i++) {
        ...
        try {
          ...
          // Expected due to concurrent sync attempts
        }
    ...
    it('should maintain performance under load', async () => {
      ...
      // Run operations sequentially to avoid sync-in-progress errors
      const results: number[] = [];
      ...
      expect(results.length).toBe(concurrentOperations); // All operations completed
    });
  ...
  describe('Scalability Tests', () => {
    it('should scale linearly with load', async () => {
      ...
      const timeGrowthRate = scalabilityResults
        .map((r, i) =>
      ...
      const avgGrowthRate =
        timeGrowthRate.reduce((a, b) => a + b, 0) / timeGrowthRate.length;
    ...
    it('should handle concurrent operations efficiently', async () => {
      ...
      const highConcurrencyResult = concurrencyResults.find(
        (r) => r.concurrency === 50
      );
