...
vi.mock('../../../src/utils/beads.js', () => ({
  BeadsClientImpl: vi.fn().mockImplementation(() => ({
...
describe('AsyncSyncEngine', () => {
  ...
  describe('executeSync', () => {
    ...
    it('should throw error when sync already in progress', async () => {
      ...
      await expect(syncEngine.executeSync(options)).rejects.toThrow(
  ...
  describe('validateSyncOptions', () => {
    it('should validate missing direction', async () => {
      ...
      await expect(syncEngine.executeSync(options)).resolves.toMatchObject({
        ...
        errors: expect.arrayContaining([
          expect.stringContaining('Sync direction is required'),
        ]),
      });
    });
    ...
    it('should validate invalid since option', async () => {
      ...
      await expect(syncEngine.executeSync(options)).resolves.toMatchObject({
        success: false,
        errors: expect.arrayContaining([
          expect.stringContaining('Since option must be a valid Date'),
        ]),
      });
    });
    ...
    it('should validate batchSize too small', async () => {
      ...
      expect(result.errors).toEqual(
        expect.arrayContaining([
          expect.stringContaining('Batch size must be between 1 and 1000'),
        ])
      );
    });

    it('should accept valid batchSize boundaries', async () => {
      const options1: SyncOptions = {
        direction: 'bidirectional',
        batchSize: 1,
      };
      const options2: SyncOptions = {
        direction: 'bidirectional',
        batchSize: 1000,
      };
  ...
  describe('Error Handling', () => {
    it('should handle event bus errors gracefully', async () => {
      ...
      await expect(syncEngine.executeSync(options)).resolves.toMatchObject({
        success: false,
        errors: expect.arrayContaining([
          expect.stringContaining('Event bus error'),
        ]),
      });
    });
