...
import {
  ...
  CachedSyncResult,
  SyncConflict,
} from "../../../src/sync/CachedSyncEngine.js";
...
describe("CachedSyncEngine", () => {
  ...
  beforeEach(async () => {
    ...
    const { CachedGitHubClient } =
      await import("../../../src/utils/github/CachedGitHubClient.js");
    const { CachedBeadsClient } =
      await import("../../../src/utils/beads/CachedBeadsClient.js");
  ...
  describe("Cache Management", () => {
    ...
    it("should export cache data", async () => {
      ...
      expect(mockGitHubClient.exportCache).toHaveBeenCalledWith(
        `${exportPath}.github`,
      );
      expect(mockBeadsClient.exportCache).toHaveBeenCalledWith(
        `${exportPath}.beads`,
      );
    });
    ...
    it("should import cache data", async () => {
      ...
      expect(mockGitHubClient.importCache).toHaveBeenCalledWith(
        `${importPath}.github`,
      );
      expect(mockBeadsClient.importCache).toHaveBeenCalledWith(
        `${importPath}.beads`,
      );
    });
  });
  ...
  describe("Background Cleanup", () => {
    it("should cleanup expired caches", async () => {
      const cleanupSpy = vi
        .spyOn(cacheManager, "cleanupExpired")
        .mockResolvedValue();
