...
describe('DevServer', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(process, 'on').mockImplementation(
      (event: string | symbol, listener: any) => {
        // Store the listener for later retrieval in tests
        mockCalls.push([event, listener]);
        return originalProcessOn.call(process, event, listener);
      }
    );
    (process.on as any).mockCalls = mockCalls;
  ...
  describe('Shutdown', () => {
    it('should cleanup processes and watcher on shutdown', () => {
      ...
      // Trigger shutdown
      try {
        const mockCalls = (process.on as any).mockCalls || [];
        const sigintCall = mockCalls.find((call: any) => call[0] === 'SIGINT');
