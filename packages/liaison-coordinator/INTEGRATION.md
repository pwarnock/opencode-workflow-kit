...
```typescript
import { CodyBeadsIntegration } from '@pwarnock/liaison';
...
```bash
# Initialize a new project with Liaison integration
bunx @pwarnock/liaison init

# Interactive setup
bunx @pwarnock/liaison init --interactive

# Use specific template
bunx @pwarnock/liaison init --template react-node
```
...
```bash
# Sync all changes
bunx @pwarnock/liaison sync

# Sync specific direction
bunx @pwarnock/liaison sync --direction cody-to-beads

# Sync with conflict resolution
bunx @pwarnock/liaison sync --resolve-conflicts
```
...
bunx @pwarnock/liaison template list

# Apply template to current project
bunx @pwarnock/liaison template apply react-native

# Create custom template
bunx @pwarnock/liaison template create my-stack
```
...
bunx @pwarnock/liaison config validate

# Show current configuration
bunx @pwarnock/liaison config show

# Update configuration
bunx @pwarnock/liaison config set sync.interval 60000
```
...
#### React Node Stack
```bash
npx @pwarnock/liaison init --template react-node
```
...
```bash
npx @pwarnock/liaison init --template python-science
```
...
```bash
npx @pwarnock/liaison init --template react-native
```
...
#### Django (Coming Soon)
```bash
npx @pwarnock/liaison init --template django
```
...
#### Rust (Coming Soon)
```bash
npx @pwarnock/liaison init --template rust
```
...
```typescript
import { Plugin, PluginContext } from '@pwarnock/liaison';
