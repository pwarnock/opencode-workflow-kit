/**
 * OpenCode Liaison Integration Package
...
export {
  ...
  PluginMetadata,
  PluginHealth,
} from "./core/plugin-system/base.js";
...
export const PACKAGE_INFO = {
  ...
  homepage:
    "https://github.com/pwarnock/opencode-workflow-kit/tree/main/packages/liaison",
} as const;
...
export async function quickSetup(
  ...
  const { ConfigurationValidator } =
    await import("./core/config/validation.js");
  ...
  const storage = {
    ...
    set: async (key: string, value: any) => {
      (global as any)._quickSetupStorage =
        (global as any)._quickSetupStorage || {};
  ...
  const syncEngine = new (SyncEngine as any)(
    config,
    validator,
    logger,
    storage,
  ...
  if (!validation.valid) {
    throw new Error(
      `Configuration validation failed: ${validation.errors.map((e: any) => e.message).join(", ")}`,
    );
  }
