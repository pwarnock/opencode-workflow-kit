...
cacheCommand
  ...
  .option(
    '--backend <type>',
    'Cache backend type (memory|disk|hybrid)',
    'hybrid'
  )
  .action(async (options) => {
    ...
    if (options.json) {
      ...
      const statsTable = [
        ...
        {
          Metric: 'Hit Rate',
          Value: `${((stats.hits / (stats.hits + stats.misses)) * 100 || 0).toFixed(1)}%`,
        },
        { Metric: 'Evictions', Value: stats.evictions.toLocaleString() },
        {
          Metric: 'Total Size',
          Value: `${stats.totalSize.toLocaleString()} entries`,
        },
        {
          Metric: 'Memory Size',
          Value: `${(stats.memorySize / 1024 / 1024).toFixed(2)} MB`,
        },
        {
          Metric: 'Disk Size',
          Value: `${(stats.diskSize / 1024 / 1024).toFixed(2)} MB`,
        },
      ];
  ...
  .action(async (pattern, options) => {
    ...
    try {
      if (pattern) {
        ...
        console.log(
          chalk.blue(
            `🗑️ Clearing ${matchingKeys.length} cache entries matching pattern: ${pattern}`
          )
        );
        ...
        console.log(
          chalk.green(
            `✅ Cleared ${matchingKeys.length} entries in ${duration.toFixed(2)}ms`
          )
        );
      } else {
        ...
        const duration = performance.now() - startTime;
        console.log(
          chalk.green(`✅ Cache cleared in ${duration.toFixed(2)}ms`)
        );
      }
    } catch (error) {
      console.error(
        chalk.red(`❌ Error clearing cache: ${(error as Error).message}`)
      );
      process.exit(1);
    }
  });
...
cacheCommand
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.log(
        `Final hit rate: ${((finalStats.hits / (finalStats.hits + finalStats.misses)) * 100 || 0).toFixed(1)}%`
      );
      ...
      console.error(
        chalk.red(`❌ Error warming cache: ${(error as Error).message}`)
      );
      process.exit(1);
    }
  });
...
cacheCommand
  ...
  .action(async (file, options) => {
    ...
    try {
      ...
      console.error(
        chalk.red(`❌ Error exporting cache: ${(error as Error).message}`)
      );
      process.exit(1);
    }
  });
...
cacheCommand
  ...
  .action(async (file, options) => {
    ...
    try {
      ...
      console.log(`Total entries: ${finalStats.totalSize.toLocaleString()}`);
      console.log(
        `Hit rate: ${((finalStats.hits / (finalStats.hits + finalStats.misses)) * 100 || 0).toFixed(1)}%`
      );
    } catch (error) {
      console.error(
        chalk.red(`❌ Error importing cache: ${(error as Error).message}`)
      );
      process.exit(1);
    }
  });
...
cacheCommand
  ...
  .action(async (options) => {
    ...
    console.log(
      chalk.green(`  ✅ Write: ${writeOpsPerSecond.toFixed(0)} ops/sec`)
    );
    ...
    console.log(
      chalk.green(`  ✅ Read: ${readOpsPerSecond.toFixed(0)} ops/sec`)
    );
    ...
    console.log(
      chalk.green(`  ✅ Mixed: ${mixedOpsPerSecond.toFixed(0)} ops/sec`)
    );
    ...
    const avgPerformance =
      (writeOpsPerSecond + readOpsPerSecond + mixedOpsPerSecond) / 3;
    let rating = 'Poor';
...
// Helper functions
function analyzeKeyDistribution(
  keys: string[]
): Array<{ Type: string; Count: number; Percentage: string }> {
