...
/**
 * Simple release script for @pwarnock/liaison
 * Creates a release without complex build dependencies
 */

import { readFileSync, writeFileSync, existsSync } from 'fs';
import { execSync } from 'child_process';
import { getPackageName } from '../src/config/package-metadata.js';

const packageJson = JSON.parse(readFileSync('./package.json', 'utf8'));

console.log(`🚀 Preparing ${getPackageName()} release...`);
...
try {
  const helpOutput = execSync('node bin/liaison.js --help', {
    encoding: 'utf8',
    timeout: 10000 
  });
  
  if (helpOutput.includes('liaison') && helpOutput.includes('sync')) {
...
try {
  ...
  const testOutput = execSync(`cd temp-test-install && npx ${getPackageName().split('/')[1]} --help`, {
    encoding: 'utf8',
    timeout: 10000
  });
  
  if (testOutput.includes(getPackageName().split('/')[1]) && testOutput.includes('sync')) {
