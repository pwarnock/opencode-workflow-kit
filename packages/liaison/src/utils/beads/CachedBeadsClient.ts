...
export class CachedBeadsClient {
  ...
  async updateWorkspace(updates: Partial<BeadsWorkspace>): Promise<BeadsWorkspace> {
    const response = await this.makeRequest(`/workspaces/${this.workspaceId}`, {
      method: 'PUT',
      body: updates
    });
  ...
  async createIssue(issue: Omit<BeadsIssue, 'id' | 'created_at' | 'updated_at'>): Promise<BeadsIssue> {
    const response = await this.makeRequest(`/workspaces/${this.workspaceId}/issues`, {
      method: 'POST',
      body: issue
    });
  ...
  async createSyncResult(syncResult: Omit<BeadsSyncResult, 'id' | 'started_at' | 'completed_at'>): Promise<BeadsSyncResult> {
    const response = await this.makeRequest(`/workspaces/${this.workspaceId}/syncs`, {
      method: 'POST',
      body: syncResult
    });
  ...
  async getSyncResult(syncId: string): Promise<BeadsSyncResult> {
    ...
    return await this.cache.getCachedBeadsData(
      cacheKey,
      async () => {
        const response = await this.makeRequest(`/workspaces/${this.workspaceId}/syncs/${syncId}`);
  ...
  async getTags(): Promise<Array<{ id: string; name: string; color: string; count: number }>> {
    ...
    return await this.cache.getCachedBeadsData(
      cacheKey,
      async () => {
        const response = await this.makeRequest(`/workspaces/${this.workspaceId}/tags`);
        return response as Array<{ id: string; name: string; color: string; count: number }>;
      }
    );
  }

  // Batch operations
  async batchCreateIssues(issues: Array<Omit<BeadsIssue, 'id' | 'created_at' | 'updated_at'>>): Promise<BeadsIssue[]> {
    const response = await this.makeRequest(`/workspaces/${this.workspaceId}/issues/batch`, {
      method: 'POST',
      body: { issues }
    });
  ...
  async batchUpdateIssues(updates: Array<{ id: string; updates: Partial<BeadsIssue> }>): Promise<BeadsIssue[]> {
    const response = await this.makeRequest(`/workspaces/${this.workspaceId}/issues/batch`, {
      method: 'PUT',
      body: { updates }
    });
  ...
  private async makeRequest(endpoint: string, options: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Beads API error: ${response.status} ${response.statusText} - ${errorText}`);
    }
  ...
  async monitorSyncHealth(): Promise<{
    ...
    const successfulSyncs = recentSyncs.filter(sync => sync.status === 'completed');
