...
export class ErrorFactory {
  static create(code: ErrorCode, message: string, details?: any): OpenCodeError {
    return new OpenCodeError(message, code, details);
  }

  static workflow = {
    registrationFailed: (message: string, details?: any) => 
      new OpenCodeError(message, ErrorCode.WORKFLOW_REGISTRATION_FAILED, details),
    unregistrationFailed: (message: string, details?: any) => 
      new OpenCodeError(message, ErrorCode.WORKFLOW_UNREGISTRATION_FAILED, details),
