...
export class BeadsClientImpl implements BeadsClient {
  ...
  private async runBeadsCommand(args: string[], projectPath?: string, input?: any): Promise<any> {
  ...
  async getIssues(projectPath: string, options?: { since?: Date }): Promise<BeadsIssue[]> {
    try {
      ...
      console.warn(chalk.yellow(`⚠️  Failed to get issues from Beads: ${error}`));
      return [];
    }
  }

  async createIssue(projectPath: string, issue: Partial<BeadsIssue>): Promise<BeadsIssue> {
    const args = [
      'create',
      issue.title || '',
      '--json'
  ...
  async updateIssue(projectPath: string, issueId: string, update: Partial<BeadsIssue>): Promise<BeadsIssue> {
    const args = [
      'update',
      issueId,
      '--json'
  ...
  async createComment(projectPath: string, issueId: string, comment: Partial<BeadsComment>): Promise<BeadsComment> {
    const args = [
      'comment',
      'create',
      issueId,
  ...
  async updateComment(projectPath: string, issueId: string, commentId: string, comment: Partial<BeadsComment>): Promise<BeadsComment> {
    const args = [
      'comment',
      'update',
      issueId,
  ...
  async deleteComment(projectPath: string, issueId: string, commentId: string): Promise<void> {
    const args = [
      'comment',
      'delete',
      issueId,
  ...
  async addLabel(projectPath: string, issueId: string, label: string): Promise<void> {
    const args = [
      'label',
      'add',
      issueId,
  ...
  async removeLabel(projectPath: string, issueId: string, label: string): Promise<void> {
    const args = [
      'label',
      'remove',
      issueId,
