  ...
  .action(async (action, identifier, options) => {
    try {
      switch (action) {
        ...
        case 'create':
          if (!identifier) {
            console.error(chalk.red('❌ Version identifier is required for create action'));
            process.exit(1);
          }
        ...
        case 'release':
          if (!identifier) {
            console.error(chalk.red('❌ Version identifier is required for release action'));
            process.exit(1);
          }
...
async function createVersion(identifier: string, features?: string, type?: string): Promise<void> {
  ...
  await fs.writeFile(path.join(versionDir, 'feature-backlog.md'), featureBacklog);
...
async function releaseVersion(identifier: string): Promise<void> {
  ...
  if (versionData.status !== 'built') {
    console.error(chalk.red(`❌ Version ${identifier} must be built before release`));
    process.exit(1);
  }
